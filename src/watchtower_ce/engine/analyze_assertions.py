...
assertions = [
    ...
    "SELECT cvv, full_pan FROM operations.transactions WHERE cvv IS NOT NULL OR full_pan IS NOT NULL",
    ...
    "SELECT * FROM information_schema.tables WHERE table_schema = 'operations' AND table_name NOT IN ('audit_logs', 'cardholder_data', 'transactions', 'users')",
    "SELECT * FROM operations.audit_logs WHERE event_type NOT IN ('LOGIN', 'LOGOUT', 'DATA_ACCESS')",
