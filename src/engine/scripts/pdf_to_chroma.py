...
class PDFToChroma:
	...
	def __init__(
		...
		self.collection_name = collection_name or self.pdf_path.stem  # default: filename
	...
	def _load_pdf(self) -> list[Document]:
		...
		print(f"[INFO] Successfully loaded {len(documents)} pages from {self.pdf_path.name}.")
	...
	def _handle_existing_collection(self) -> bool:
		...
		try:
			...
			if self.overwrite:
				with yaspin(Spinners.arc, text=f'[INFO] Deleting existing collection "{self.collection_name}"...'):
					client.delete_collection(self.collection_name)
				print(f'[INFO] Successfully deleted old collection "{self.collection_name}".')
			else:
				warnings.warn(
	...
	def build(self) -> None:
		...
		if self.overwrite or not collection_exists:
			...
			with yaspin(Spinners.arc, text=f'[INFO] Creating collection "{self.collection_name}"...'):
